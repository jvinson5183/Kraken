...
function AICommandPanel({ 
  ...
  useEffect(() => {
    const handleKeydown = (event: KeyboardEvent) => {
      // Try multiple ways to detect the "3" key
      const isThreeKey = event.key === '3' || event.key === '#' || event.code === 'Digit3' || event.keyCode === 51
      
      if (event.ctrlKey && event.shiftKey && isThreeKey) {
        console.log('🔧 Ctrl+Shift+3 detected, toggling minimize state')
        event.preventDefault()
        event.stopPropagation()
        setIsMinimized(prev => {
          console.log('📏 Minimized state changing from:', prev, 'to:', !prev)
          return !prev
        })
      }
    }

    // Add listener to document to ensure it captures all events
  ...
  return (
    <motion.div
      className={`w-96 ${isMinimized ? 'h-auto' : 'h-96'} bg-gray-900/95 backdrop-blur-md border border-gray-700 rounded-lg shadow-2xl flex flex-col`}
...
export function KrakenAssistant({ 
  ...
  // Handle immediate alert message
  useEffect(() => {
    if (immediateMessage) {
      console.log('🤖 Adding immediate alert message to conversation:', immediateMessage)
      
      const krakenMessage: ChatMessage = {
        id: Date.now().toString() + '_immediate',
        type: 'kraken',
